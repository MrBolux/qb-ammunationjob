...
local function deliveryProgress()
    ...
    QBCore.Functions.Progressbar("delivery_process", 'Livraison en cours...', math.random(3000, 4000), false, true, {
        ...
        TriggerServerEvent("qb-ammunationjob:server:deliveryItem", deliveryItem)
        task = false
	...
	while delivery do
		if task then
			...
			if deliveryCount < limitCollect then
                ...
                TriggerEvent('qb-ammunationjob:client:stopDelivery')
...
RegisterNetEvent('qb-ammunationjob:client:startDelivery', function()
    delivery = true
    startDelivery()
end)

RegisterNetEvent('qb-ammunationjob:client:stopDelivery', function()
    delivery = false
    task = false
    deliveryCount = 0
    exports['togolo_lib']:deleteInteractPed(deliveryPed)
